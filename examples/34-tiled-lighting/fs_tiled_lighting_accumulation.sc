...
BUFFER_RO(outLightIndices, float, 1);

uniform vec4 u_dispatchParams; // hlsl doesn't seem to make this available
uniform vec4 u_diffuseColor;
uniform vec4 u_ambientColor;

// a horrible clamping hack to get lights with finite support
float clamp_light(vec3 lightDirection, float radius) {
...
void main() {
    ...
    // if you want a faceted look (something like face normals) do this instead
    ...
    for (uint i = 0; i < MAX_LIGHTS_PER_TILE && outLightIndices[offset + i] >= 0.0; ++i) {
        uint lightIndex = uint(outLightIndices[offset + i]) * LIGHT_STRIDE;
        ...
        // Calculate the light clamping on the pre-normalized lightDirection
        vec3 lightDirection = lightPos.xyz - v_wpos.xyz;
        float clamping = clamp_light(lightDirection, lightRadius);

        // Basic lambertian contribution
        lightDirection = normalize(lightDirection);
        float dp = max(dot(lightDirection, worldNormal), 0.0);
        color += lightColor.xyz * dp * clamping;
    }
